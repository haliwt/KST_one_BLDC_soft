...
void KEY_Init(void)
{
	...
	GPIO_Initure.Pin=GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_10|GPIO_PIN_12
		              |GPIO_PIN_14|GPIO_PIN_11|GPIO_PIN_15; //GPIO_B
    GPIO_Initure.Mode=GPIO_MODE_INPUT;      //输入
	...
	GPIO_Initure.Pin=GPIO_PIN_9;            //GPIO_D
    GPIO_Initure.Mode=GPIO_MODE_INPUT;      //输入
    GPIO_Initure.Pull=GPIO_PULLUP;        //下拉
    GPIO_Initure.Speed=GPIO_SPEED_HIGH;     //高速
...
uint8_t KEY_Scan(uint8_t mode)
{
    ...
    if(key_up&&(START==0||BRAKE==0||KEY2==0
        ...
        ||PUMP==0||TURN_LIGHT==0||HALL_SW==0||GLOBAL==0||SPEED_ADD ==0 || SPEED_REDUCE ==0)
    {
        HAL_Delay(10);
        key_up=0;
        if(START==0)                             return  START_PRES;
             ...
             &&PUMP==1 && TURN_LIGHT==1 && HALL_SW==1 && GLOBAL == 1
